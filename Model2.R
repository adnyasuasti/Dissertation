...
# Defining dynamic of vaccine reducing probability of clinical manifestation model ####
sis <- function(t,Y,p){
  # Parameters #################################################################
...
p.vaccine = c(
  ...
  x1      = 0.0883,     # Risk factor of low-risk individuals
  x2      = 4.6467,     # Risk factor of high-risk individuals
  eta     = 1           # reduced factor for vaccinated individuals
)
...
aux = 200 # 200 equations for each compartment
init = rep(0,6*aux)
...
sol_v0               = sol
sol_v0[,2:ncol(sol)] = 0
n                    = abs(round(log(1-0.3)/0.0285))

# Event of vaccination
for (i in 2:(aux-n)) {
  sol_v0[,00*aux+i+n] = sol[,00*aux+i]
  sol_v0[,01*aux+i+n] = sol[,01*aux+i]
  sol_v0[,02*aux+i+n] = sol[,02*aux+i]
  sol_v0[,03*aux+i+n] = sol[,03*aux+i]
  sol_v0[,04*aux+i+n] = sol[,04*aux+i]
  sol_v0[,05*aux+i+n] = sol[,05*aux+i]
}
...
for (i in 1:80) {
  ...
  sol_v1[i+1,] = lsoda(
    ...
    y     = as.numeric(sol_v0[i,2:ncol(sol)]),
    # Simulation for one year
    times = t,
    func  = sis,
...
cases.cum_v1 = rowSums(
  (
    p["p1"]*(sol_v1[1:81,c((00*aux+1):(01*aux))+1]+sol_v1[1:81,c((01*aux+1):(02*aux))+1]+sol_v1[1:81,c((02*aux+1):(03*aux))+1]) + 
    p["p2"]*(sol_v1[1:81,c((03*aux+1):(04*aux))+1]+sol_v1[1:81,c((04*aux+1):(05*aux))+1]+sol_v1[1:81,c((05*aux+1):(06*aux))+1])
  )*matrix(rep(c(0:(aux-1)), each = 81), nrow = 81, ncol = aux)
)-n

# Number of cumulative cases by age - with vaccination
cases.cum_v0 = rowSums(
  (
    p["p1"]*(sol_v0[1:81,c((00*aux+1):(01*aux))+1]+sol_v0[1:81,c((01*aux+1):(02*aux))+1]+sol_v0[1:81,c((02*aux+1):(03*aux))+1]) + 
    p["p2"]*(sol_v0[1:81,c((03*aux+1):(04*aux))+1]+sol_v0[1:81,c((04*aux+1):(05*aux))+1]+sol_v0[1:81,c((05*aux+1):(06*aux))+1])
  )*matrix(rep(c(0:(aux-1)), each = 81), nrow = 81, ncol = aux)
)-n
...
# Number of individuals I and A assuming that all individuals were vaccinated
theta_v1 = sum(
  rowSums(
    p["p1"]*( # Low-risk group
      infectious.time1*sol_v1[1:81,c((01*aux+1):(02*aux))+1]+   #Iv1,i
                   rho*sol_v1[1:81,c((02*aux+1):(03*aux))+1]    #Av1,i
    ) +
    p["p2"]*( # High-risk group
      infectious.time1*sol_v1[1:81,c((04*aux+1):(05*aux))+1]+   #Iv2,i
                   rho*sol_v1[1:81,c((05*aux+1):(06*aux))+1]    #Av2,i
    )
  )*age_distribution
)
...
# Specify the number of colors in the colourmap
...
eta_p_fullvac = ( theta_v1*k + theta*(1-k) ) / theta
eta_p_fullvac

# Age min and max for vaccinated individuals
age.vac.min = 0
age.vac.max = 17 + k*10/2
...
# Sol_v1.aux: to avoid considering that individuals had extra n malaria episodes
sol_v1.aux    = sol_v1
sol_v1.aux[,] = 0
for (i in (n+2):aux) {
  sol_v1.aux[,00*aux+i-n] = sol_v1[,00*aux+i]
  sol_v1.aux[,01*aux+i-n] = sol_v1[,01*aux+i]
  sol_v1.aux[,02*aux+i-n] = sol_v1[,02*aux+i]
  sol_v1.aux[,03*aux+i-n] = sol_v1[,03*aux+i]
  sol_v1.aux[,04*aux+i-n] = sol_v1[,04*aux+i]
  sol_v1.aux[,05*aux+i-n] = sol_v1[,05*aux+i]
}
...
sol_f = cbind( 
    ...
    # Low-risk group
    rbind(k*sol_v1.aux[ aux.age.vac,c((00*aux+1):(03*aux))+1]+
      (1-k)*sol_p[      aux.age.vac,c((00*aux+1):(03*aux))+1],
              sol_p[     -aux.age.vac,c((00*aux+1):(03*aux))+1]),   #c(Sv1,i:A1,i)
    # High-risk group
    rbind(k*sol_v1.aux[ aux.age.vac,c((03*aux+1):(06*aux))+1]+
...
incidence = data.frame(
  ...
  vacination      = abs(cases.cum_v1[2:81] - cases.cum[1:80]),
  no.vacination.indirect = (cases.cum_p[2:81] - cases.cum[1:80]),
  final.case      = abs(cases.cum_f[2:81]  - cases.cum_v0[1:80])
)

FILENAME = 'out10/model2_children17_n3'
...
mtext("A", side = 3, line =-2, adj = 0, cex=1.5, outer= TRUE)
dev.off()
